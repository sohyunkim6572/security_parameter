...
public:
  ...
  {
    //std::cout << "[sub callback] PID: " << syscall(__NR_gettid) << std::endl;
    ++messages_received;
...
private:
  ...
  void * physics_update()
  {
    ...
    std::cout << "[physics_update] Thread: " << syscall(__NR_gettid) << std::endl;
    while (!done_) {
      ...
      //std::cout << "[physics_update] PID: " << syscall(__NR_gettid) << std::endl;
      //sensor_message_.num = data_num2++;
