        ...
        Locator input_locator)
{
    ...
    std::cout << "[2] Thread: " << syscall(__NR_gettid) << " port: " << input_locator.port << std::endl;
    ...
    while (alive())
    {
        ...
        {
	    //syscall(459, input_locator.port);
    	    //printf("UDPChannelResource::perform_listen_operation TID: %ld\n", syscall(__NR_gettid));
            message_receiver()->OnDataReceived(msg.buffer, msg.length, input_locator, remote_locator);
	    //syscall(459, remote_locator.port);
        }
        else if (alive())
        {
