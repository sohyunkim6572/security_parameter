...
void DataSharingListener::run()
{
    ...
    std::cout << "[3] Thread: " << syscall(__NR_gettid) << " DataSharingListener run 1" << std::endl;
    while (is_running_.load())
    {
        ...
        do
        {
    	    ...
    	    std::cout << "[3] Thread: " << syscall(__NR_gettid) << " DataSharingListener run 2" << std::endl;
            process_new_data();
...
void DataSharingListener::start()
{
    ...
    std::cout << "[3] Thread: " << syscall(__NR_gettid) << " DataSharingListener start" << std::endl;
...
void DataSharingListener::process_new_data ()
{
    ...
    for (auto it = writer_pools_.begin(); it != writer_pools_.end(); ++it)
    {
        ...
        while (has_new_payload)
        {
    		...
    		printf("DataSharingListener::process_new_data TID: %ld\n", syscall(__NR_gettid));
